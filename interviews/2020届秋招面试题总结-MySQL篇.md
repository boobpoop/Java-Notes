...
[select关键字执行顺序](https://www.cnblogs.com/kingforn/articles/11674475.html)
...
水平切分,是将同一个表中的记录拆分到多个结构相同的表中。当一个表的数据不断增多时，水平拆分是必然的选择，它可以将数据分布到􏱹群的不同节点上，从而缓存单个数据
库的压力。

垂直切分是将一张表按列切分成多个表，通常是按照列的关系密程度进行切分，也可以利用垂直切分将经常被使用的列和不经常被使用的列切分到不同的表中。
在数据库的层面使用垂直切分将按数据库中表的密集程度布署到不同的库中，例如将原来的电商数据库垂直切分成商品数据库和用户数据库。
...
[in和exists区别](https://www.cnblogs.com/clarke157/p/7912871.html)

**16、数据库自增主键可能的问题。**

自增长是一个很常见的数据属性，在MySQL中我很喜欢让一个自增长属性的字段（比如ID）当作一个主键。特别是InnoDB，因为InnoDB的聚集索引特性，自增能保证永不重复，这是主键要求的天然属性。使用自增长属性的字段当主键性能更好。但是也存在一些问题。
...
MVCC，是一种用来解决读-写冲突的无锁并发控制，也就是为事务分配单向增长的时间戳，为每个修改保存一个版本，版本与事务时间戳关联，读操作只读该事务开始前的数据库的快照（undo log）。 这样在读操作不用阻塞写操作，写操作不用阻塞读操作的同时，避免了脏读和不可重复读。
...
业务代码使用sharding-jdbc的jar包后，配置分片规则，解析sql，改写sql并路由。

[中间件分库分表](https://blog.csdn.net/fly910905/article/details/87101059)

**19、MySQL的主从延迟怎么解决。**

生产环境中经常会遇到MySQL主从延迟问题，从原理上也能看出主库的事务提交是并发模式，而从库只有一个SQL线程负责解析，所以本身上就可能存在延迟。
...
参考文章：[Mybatis如何防止SQL注入](https://blog.csdn.net/yizhenn/article/details/52384601)
...
在select语句中可以使用groupby子句将行划分成较小的组，然后，使用聚组函数（SQL基本函数，聚合函数对一组值执行计算，并返回单个值，也被称为组函数。 聚合函数经常与 SELECT 语句的 GROUP BY 子句的HAVING一同使用。）返回每一个组的汇总信息，另外，可以使用having子句限制返回的结果集。HAVING语句的存在弥补了WHERE关键字不能与聚合函数联合使用的不足。
...
[having和on的区别](https://blog.csdn.net/qq_36981835/article/details/54581481)
...
innodb和myisam都是用的b+树，只是myisam将索引和数据分开存储。
...
**第三范式，是要求一个数据库表中不存在传递依赖。** 例如，存在一个部门信息表，其中每个部门有部门编号（dept_id）、部门名称、部门简介等信息。那么在的员工信息表中列出部门编号后就不能再将部门名称、部门简介等与部门有关的信息再加入员工信息表中。如果不存在部门信息表，则根据第三范式（3NF）也应该构建它，否则就会有大量的数据冗余。简而言之，第三范式就是属性不依赖于其它非主属性。 也就是说， 如果存在非主属性对于码的传递函数依赖，则不符合3NF的要求。
